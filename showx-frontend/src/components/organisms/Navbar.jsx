  ...
  return (
    <nav className={`sticky top-0 z-50 border-b backdrop-blur-2xl transition-all duration-300 ${
      ...
      <div className="max-w-[1440px] mx-auto px-6 h-20 flex items-center justify-between gap-6">
        ...
        <motion.div 
          animate={{ flexGrow: searchFocused ? 0.6 : 0.4 }}
          className="hidden md:flex items-center relative group"
        >
          <Search size={16} className={`absolute left-3.5 transition-colors duration-300 ${searchFocused ? 'text-amber-500' : 'text-slate-500'}`} />
          <input
            type="text"
            placeholder={currentText}
            onFocus={() => setSearchFocused(true)}
            onBlur={() => setSearchFocused(false)}
            className={`w-full border rounded-xl pl-11 pr-4 py-2.5 text-sm transition-all duration-300 shadow-inner focus:outline-none focus:ring-4 focus:ring-amber-500/5 ${
              isDarkMode 
                ? "bg-white/[0.03] hover:bg-white/[0.05] border-white/[0.06] text-white focus:border-amber-500/40" 
                : "bg-stone-50 hover:bg-stone-100/70 border-stone-200 text-stone-900 focus:border-amber-500/40"
            }`}
          />
        </motion.div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            to={selectedSeats.length > 0 ? "/checkout" : "#"}
            className={`relative p-2.5 rounded-xl border ${selectedSeats.length > 0 ? "border-amber-500/30 bg-amber-500/10 text-amber-600" : "border-white/[0.04] text-slate-600 cursor-not-allowed"}`}
          >

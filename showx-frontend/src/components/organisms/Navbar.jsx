  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        // Fallback default active states
      }
    };
    fetchCities();
  }, []);
  ...
  return (
    <>
      <nav className={`sticky top-0 z-50 border-b backdrop-blur-2xl transition-all duration-300 ${
        ...
        <div className="max-w-[1440px] mx-auto px-3 sm:px-6 h-20 flex items-center justify-between gap-2 sm:gap-6">
          
          {/* BRAND LAYOUT LOGO ROW - Fixed text bounds to ensure full brand context maps beautifully without trailing shifts */}
          <Link to="/" className="flex items-center gap-2 group shrink-0 select-none">
            ...
            <div className="flex flex-col justify-center">
              <span className={`font-display text-lg sm:text-2xl font-black tracking-tight transition-colors leading-none ${
                isDarkMode ? "text-white" : "text-stone-900"
              }`}>
                Showx
              </span>
              <span className="bg-gradient-to-r from-amber-600 via-amber-500 to-yellow-600 bg-clip-text text-transparent font-mono text-[8px] sm:text-[10px] font-black tracking-widest uppercase mt-0.5 sm:mt-1">
                The CinemaHub
              </span>
            </div>
          </Link>

          {/* Desktop Search Component Node */}
          <div 
          ...
          {/* ACTION BUTTON WRAPPER - Refitted with compact flex paddings to securely scale down to 320px screens */}
          <div className="flex items-center gap-1.5 sm:gap-4 shrink-0">
            ...
            <div className="relative">
              <button 
                ...
                className={`flex items-center gap-1 px-2 py-2 rounded-xl text-xs font-bold transition-all border cursor-pointer ${
                  isDarkMode ? "bg-white/[0.02] border-white/[0.05] text-slate-300" : "bg-stone-50 border-stone-200 text-stone-700"
                }`}
              >
                <MapPin size={13} className="text-amber-600 shrink-0" />
                <span className="max-w-[42px] sm:max-w-none truncate">{selectedCity || 'Karnal'}</span>
                <ChevronDown size={11} className={`transition-transform duration-200 shrink-0 ${isLocationOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                {isLocationOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 8 }}
                      className={`absolute right-0 mt-2 w-40 border rounded-xl p-1 shadow-2xl z-20 ${
                    ...
                    >
                      {cities.map((city) => (
                        <button
                          ...
                          className={`w-full text-left px-2.5 py-2 text-xs font-semibold rounded-lg cursor-pointer border-none bg-transparent ${
            ...
            {isLoggedIn && (
              <div className="relative">
                ...
                >
                  ...
                  {unreadCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-red-500 text-white text-[8px] font-black flex items-center justify-center border border-white">
                  ...
                  {isNotifOpen && (
                    <>
                      ...
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 8 }}
                        className={`absolute right-0 mt-2 w-64 sm:w-80 max-h-96 overflow-y-auto border rounded-2xl shadow-2xl z-20 ${
                          isDarkMode ? "bg-slate-900 border-white/[0.06]" : "bg-white border-stone-200"
                        }`}
                      >
            ...
            {/* Mobile Hamburger Activator Link Toggle */}
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className={`lg:hidden p-2 rounded-xl border cursor-pointer focus:outline-none shrink-0 ${
                isDarkMode ? "border-white/[0.06] bg-white/[0.02] text-slate-300" : "border-stone-200 bg-stone-50 text-stone-700"
              }`}
        ...
        {/* Desktop Second Row Anchor Bar Container */}
        ...
        {/* Mobile Full Navigation Drawer Container Node */}
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              <div className="px-4 py-5 flex flex-col gap-4 text-sm font-bold">
                {isLoggedIn && (
                  <div className={`flex items-center gap-3 pb-3 border-b ${isDarkMode ? "border-white/[0.05]" : "border-stone-100"}`}>
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-amber-500 to-yellow-500 flex items-center justify-center font-black text-stone-950 select-none">
                      {getUserInitials(user.name)}
                    </div>
                    <div className="truncate">
                      <p className={`text-sm font-black truncate ${isDarkMode ? "text-white" : "text-stone-800"}`}>{user.name}</p>
                ...
                <div className="pt-2">
                  {isLoggedIn ? (
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-black text-red-500 bg-red-500/10 border-none cursor-pointer text-xs uppercase tracking-wider"
                    >
                      <LogOut size={14} /> Close Active Session
                    </button>
                  ) : (
                    <Link 
                      to="/login" 
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-black bg-amber-500 text-stone-950 shadow-md border-none text-xs uppercase tracking-wider text-center"
                    >
                      <LogIn size={14} strokeWidth={2.5} /> <span>Sign In to Portal</span>
                    </Link>
                  )}
                </div>
              </div>
